  ...
  return (
    <div className="rounded-lg bg-muted/50 p-4 space-y-2 text-sm">
      <div className="flex justify-between">
        ...
        <span className="font-medium tabular-nums">{formatSTX(amount)}</span>
      </div>
      <div className="flex justify-between">
        <span className="text-muted-foreground">Protocol Fee (1%)</span>
        <span className="font-medium tabular-nums text-warning">−{formatSTX(fee)}</span>
      </div>
      <div className="border-t border-border pt-2 flex justify-between">
        <span className="font-medium">Recipient Gets</span>
        <span className="font-bold tabular-nums text-success">{formatSTX(net)}</span>
      </div>
    </div>
  );
}
